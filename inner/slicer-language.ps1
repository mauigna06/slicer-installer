...
'@ -replace '@LANG@', $Code
    ...
    $steps = @(
        @{ Code = $extPyCode;  Message = 'Installing the SlicerLanguagePacks extension...';
           Fail = 'Could not install SlicerLanguagePacks (Slicer exited with code {0}). Launch Slicer and use its Extensions Manager instead.' },
        @{ Code = $langPyCode; Message = "Downloading the '$Code' translation and switching Slicer to it...";
           Fail = "Could not install or verify the '$Code' translation (details above; Slicer exited with code {0}). Launch Slicer and use the extension's LanguageTools module instead." }
    )
