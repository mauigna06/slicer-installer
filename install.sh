...
EOF
  ...
  $runner "$exe" --no-splash --no-main-window --python-script "$ext_pyfile" >/dev/null \
    || err "Could not install SlicerLanguagePacks. If Slicer failed to start for lack of system libraries, install them first (run slicer-deps); otherwise launch Slicer and use its Extensions Manager."
